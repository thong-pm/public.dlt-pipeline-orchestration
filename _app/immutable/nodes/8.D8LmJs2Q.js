...
where deal_stage = 'closedlost'
  ...
  and notes_last_updated_at <= '2026-06-15'::date`;o.kpi_lost_value_data&&(o.kpi_lost_value_data instanceof Error?ce.initialError=o.kpi_lost_value_data:ce.initialData=o.kpi_lost_value_data,o.kpi_lost_value_columns&&(ce.knownColumns=o.kpi_lost_value_columns));let Fe,ee=!1;const X=qt.createReactive({callback:x=>{a(2,Fe=x)},execFn:$},{id:"kpi_lost_value",...ce});X(Re,{noResolve:O,...ce}),globalThis[Symbol.for("kpi_lost_value")]={get value(){return Fe}};let Q={initialData:void 0,initialError:void 0},ue=oe`select
    count(distinct deal_id) as open_deals_count
from postgres.fct_pipeline
where deal_stage not in ('closedwon', 'closedlost')`,be=`select
    count(distinct deal_id) as open_deals_count
from postgres.fct_pipeline
where deal_stage not in ('closedwon', 'closedlost')`;o.kpi_open_deals_data&&(o.kpi_open_deals_data instanceof Error?Q.initialError=o.kpi_open_deals_data:Q.initialData=o.kpi_open_deals_data,o.kpi_open_deals_columns&&(Q.knownColumns=o.kpi_open_deals_columns));let te,$t=!1;const Me=qt.createReactive({callback:x=>{a(3,te=x)},execFn:$},{id:"kpi_open_deals",...Q});Me(be,{noResolve:ue,...Q}),globalThis[Symbol.for("kpi_open_deals")]={get value(){return te}};let B={initialData:void 0,initialError:void 0},z=oe`with client_shares as (
    select
...
select coalesce(max(share), 0) as max_concentration
from client_shares`;o.kpi_top_client_concentration_data&&(o.kpi_top_client_concentration_data instanceof Error?B.initialError=o.kpi_top_client_concentration_data:B.initialData=o.kpi_top_client_concentration_data,o.kpi_top_client_concentration_columns&&(B.knownColumns=o.kpi_top_client_concentration_columns));let Ze,Ae=!1;const et=qt.createReactive({callback:x=>{a(4,Ze=x)},execFn:$},{id:"kpi_top_client_concentration",...B});et(Ve,{noResolve:z,...B}),globalThis[Symbol.for("kpi_top_client_concentration")]={get value(){return Ze}};let P={initialData:void 0,initialError:void 0},fe=oe`with reason_ranks as (
    select
...
from reason_ranks
group by 1, 2
order by total_value desc`;o.win_loss_reasons_data&&(o.win_loss_reasons_data instanceof Error?P.initialError=o.win_loss_reasons_data:P.initialData=o.win_loss_reasons_data,o.win_loss_reasons_columns&&(P.knownColumns=o.win_loss_reasons_columns));let pe,se=!1;const de=qt.createReactive({callback:x=>{a(5,pe=x)},execFn:$},{id:"win_loss_reasons",...P});de($e,{noResolve:fe,...P}),globalThis[Symbol.for("win_loss_reasons")]={get value(){return pe}};let me={initialData:void 0,initialError:void 0},_e=oe`select
    c.name as client_name,
...
where d.deal_stage not in ('closedwon', 'closedlost')
group by all
order by pipeline_value desc`,U=`select
    c.name as client_name,
    sum(d.amount) as pipeline_value
from postgres.fct_pipeline d
join postgres.seed_xero_contacts c on d.customer_email = c.email_address
where d.deal_stage not in ('closedwon', 'closedlost')
group by all
order by pipeline_value desc`;o.client_pipeline_share_data&&(o.client_pipeline_share_data instanceof Error?me.initialError=o.client_pipeline_share_data:me.initialData=o.client_pipeline_share_data,o.client_pipeline_share_columns&&(me.knownColumns=o.client_pipeline_share_columns));let W,Y=!1;const Se=qt.createReactive({callback:x=>{a(6,W=x)},execFn:$},{id:"client_pipeline_share",...me});Se(U,{noResolve:_e,...me}),globalThis[Symbol.for("client_pipeline_share")]={get value(){return W}};let N="ytd",G="";Ha(()=>{if(localStorage.setItem("evidence-theme","light"),document.documentElement.setAttribute("data-theme","light"),l){const x=e.url.searchParams.get("time_filter");a(10,l.time_filter=x||"ytd",l)}});const yt=()=>ha(ae(`/?time_filter=${N}`)),J=()=>ha(ae(`/sales?time_filter=${N}`)),le=()=>ha(ae(`/operations?time_filter=${N}`));return s.$$set=x=>{"data"in x&&a(8,i=x.data)},s.$$.update=()=>{if(s.$$.dirty[0]&256&&a(9,{data:o={},customFormattingSettings:d,__db:k}=i,o),s.$$.dirty[0]&512&&dn.set(Object.keys(o).length>0),s.$$.dirty[1]&16&&e.params,s.$$.dirty[0]&1024&&a(12,j=oe`select
    coalesce(sum(case when deal_stage = 'closedwon' then 1 else 0 end) * 1.0 / nullif(count(case when deal_stage in ('closedwon', 'closedlost') then 1 end), 0), 0) as win_rate
from postgres.fct_pipeline
where notes_last_updated_at >= case
    when '${l.time_filter}' = 'mtd' then date_trunc('month', '2026-06-15'::date)
...
where deal_stage = 'closedlost'
  ...
  and notes_last_updated_at <= '2026-06-15'::date`),s.$$.dirty[0]&491520&&(O||!ee?O||(X(Re,{noResolve:O,...ce}),a(18,ee=!0)):X(Re,{noResolve:O})),s.$$.dirty[0]&7864320&&(ue||!$t?ue||(Me(be,{noResolve:ue,...Q}),a(22,$t=!0)):Me(be,{noResolve:ue})),s.$$.dirty[0]&125829120&&(z||!Ae?z||(et(Ve,{noResolve:z,...B}),a(26,Ae=!0)):et(Ve,{noResolve:z})),s.$$.dirty[0]&1024&&a(28,fe=oe`with reason_ranks as (
    select
        coalesce(closed_won_reason, 'Reason Undefined') as reason,
        case when deal_stage = 'closedwon' then 'Closed Won' else 'Closed Lost' end as status,
        sum(amount) as total_value,
...
from reason_ranks
group by 1, 2
order by total_value desc`),s.$$.dirty[0]&2013265920&&(fe||!se?fe||(de($e,{noResolve:fe,...P}),a(30,se=!0)):de($e,{noResolve:fe})),s.$$.dirty[1]&15&&(_e||!Y?_e||(Se(U,{noResolve:_e,...me}),a(34,Y=!0)):Se(U,{noResolve:_e})),s.$$.dirty[0]&1024&&a(0,N=(l==null?void 0:l.time_filter)||"ytd"),s.$$.dirty[0]&1&&a(7,G=En(N,qn)),s.$$.dirty[0]&1024&&l&&l.time_filter)try{const x=new URL(window.location.href);x.searchParams.get("time_filter")!==l.time_filter&&(x.searchParams.set("time_filter",l.time_filter),window.history.replaceState(null,"",x.pathname+x.search))}catch{}},a(20,ue=oe`select
    count(distinct deal_id) as open_deals_count
from postgres.fct_pipeline
where deal_stage not in ('closedwon', 'closedlost')`),a(21,be=`select
    count(distinct deal_id) as open_deals_count
from postgres.fct_pipeline
where deal_stage not in ('closedwon', 'closedlost')`),a(24,z=oe`with client_shares as (
    select
        c.name as client_name,
        sum(d.amount) as client_amount,
        sum(d.amount) * 1.0 / (select nullif(sum(amount), 0) from postgres.fct_pipeline where deal_stage not in ('closedwon', 'closedlost')) as share
...
where d.deal_stage not in ('closedwon', 'closedlost')
group by all
order by pipeline_value desc`),a(33,U=`select
    c.name as client_name,
    sum(d.amount) as pipeline_value
from postgres.fct_pipeline d
join postgres.seed_xero_contacts c on d.customer_email = c.email_address
where d.deal_stage not in ('closedwon', 'closedlost')
group by all
order by pipeline_value desc`),[N,Xe,Fe,te,Ze,pe,W,G,i,o,l,w,j,ie,ne,ce,O,Re,ee,Q,ue,be,$t,B,z,Ve,Ae,P,fe,$e,se,me,_e,U,Y,e,yt,J,le]}class Nn extends nn{constructor(t){super(),sn(this,t,Rn,Dn,Qa,{data:8},null,[-1,-1])}}export{Nn as component};
